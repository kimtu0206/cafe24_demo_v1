...
public class WebhookEventService {
    ...
    /**
     * 상품 생성/수정/삭제 이벤트 처리기.
     * eventNo + mallId + productNo 조합으로 중복 수신을 확인한 뒤, 최초 수신 시에만 action을 실행한다.
     * 생성/수정은 Cafe24에서 상품 상세를 다시 조회해 반영하고, 삭제는 로컬 DB에서만 제거한다.
     */
    @Transactional
    @EventListener
    public void onProductCreated(ProductCreatedEvent event) {
        handleProductEvent(event.getEventNo(), event.getMallId(), event.getProductNo(), WebhookEventType.PRODUCT_CREATED, "created",
    ...
    public void onProductUpdated(ProductUpdatedEvent event) {
        handleProductEvent(event.getEventNo(), event.getMallId(), event.getProductNo(), WebhookEventType.PRODUCT_UPDATED, "updated",
                () -> productService.upsertFromWebhook(event.getMallId(), event.getProductNo()));
    }
    ...
    public void onProductDeleted(ProductDeletedEvent event) {
        handleProductEvent(event.getEventNo(), event.getMallId(), event.getProductNo(), WebhookEventType.PRODUCT_DELETED, "deleted",
                () -> productService.deleteFromWebhook(event.getMallId(), event.getProductNo()));
    }

    private void handleProductEvent(Integer eventNo, String mallId, Long productNo, WebhookEventType type, String verb, Runnable action) {
        String resourceId = String.valueOf(productNo);
        if (webhookEventRepository.exists(eventNo, mallId, resourceId)) {
            log.info("Duplicate webhook ignored: eventNo={}, mallId={}, productNo={}", eventNo, mallId, productNo);
            return;
        }

        webhookEventRepository.save(eventNo, type, mallId, resourceId);
        log.info("Product {}: mallId={}, productNo={}", verb, mallId, productNo);
        action.run();
    }

    /**
     * 배송사 등록/수정/삭제 이벤트 처리기.
    ...
    public void onCarrierCreated(CarrierCreatedEvent event) {
        handleCarrierEvent(event.getEventNo(), event.getMallId(), event.getShippingCarrierCode(), WebhookEventType.CARRIER_CREATED, "created");
    }

    @Transactional
    @EventListener
    public void onCarrierUpdated(CarrierUpdatedEvent event) {
        handleCarrierEvent(event.getEventNo(), event.getMallId(), event.getShippingCarrierCode(), WebhookEventType.CARRIER_UPDATED, "updated");
    }

    @Transactional
    @EventListener
    public void onCarrierDeleted(CarrierDeletedEvent event) {
        handleCarrierEvent(event.getEventNo(), event.getMallId(), event.getShippingCarrierCode(), WebhookEventType.CARRIER_DELETED, "deleted");
    }

    private void handleCarrierEvent(Integer eventNo, String mallId, String shippingCarrierCode, WebhookEventType type, String verb) {
        if (carrierWebhookEventRepository.exists(eventNo, mallId, shippingCarrierCode)) {
            log.info("Duplicate webhook ignored: eventNo={}, mallId={}, shippingCarrierCode={}", eventNo, mallId, shippingCarrierCode);
            return;
        }

        carrierWebhookEventRepository.save(eventNo, type.name(), mallId, shippingCarrierCode);
        log.info("Carrier {}: mallId={}, shippingCarrierCode={}", verb, mallId, shippingCarrierCode);
    }

    /**
     * 혜택 등록/수정/삭제 이벤트 처리기.
     * eventNo + mallId + benefitNo 조합으로 중복 수신을 확인한 뒤, 최초 수신 시에만 action을 실행한다.
     * 생성/수정은 Cafe24에서 혜택 상세를 다시 조회해 반영하고, 삭제는 로컬 DB에서만 제거한다.
     */
    @Transactional
    @EventListener
    public void onBenefitCreated(BenefitCreatedEvent event) {
        handleBenefitEvent(event.getEventNo(), event.getMallId(), event.getBenefitNo(), WebhookEventType.BENEFIT_CREATED, "created",
    ...
    public void onBenefitUpdated(BenefitUpdatedEvent event) {
        handleBenefitEvent(event.getEventNo(), event.getMallId(), event.getBenefitNo(), WebhookEventType.BENEFIT_UPDATED, "updated",
                () -> benefitService.upsertFromWebhook(event.getMallId(), event.getBenefitNo()));
    }
    ...
    public void onBenefitDeleted(BenefitDeletedEvent event) {
        handleBenefitEvent(event.getEventNo(), event.getMallId(), event.getBenefitNo(), WebhookEventType.BENEFIT_DELETED, "deleted",
                () -> benefitService.deleteFromWebhook(event.getMallId(), event.getBenefitNo()));
    }

    private void handleBenefitEvent(Integer eventNo, String mallId, Integer benefitNo, WebhookEventType type, String verb, Runnable action) {
        String resourceId = String.valueOf(benefitNo);
        if (benefitWebhookEventRepository.exists(eventNo, mallId, resourceId)) {
            log.info("Duplicate webhook ignored: eventNo={}, mallId={}, benefitNo={}", eventNo, mallId, benefitNo);
            return;
        }

        benefitWebhookEventRepository.save(eventNo, type.name(), mallId, resourceId);
        log.info("Benefit {}: mallId={}, benefitNo={}", verb, mallId, benefitNo);
        action.run();
    }
